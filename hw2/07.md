...
*Q:Compute the response time and turnaround time when running
three jobs of length 200 with the SJF and FIFO schedulers.*

A: Same time because all jobs share same length.

* SJF:
```
E:\proj\cs\hw5600\hw2>python ./scheduler.py -p SJF -l 200,200,200 -c
ARG policy SJF
...
* FIFO:
```
E:\proj\cs\hw5600\hw2>python ./scheduler.py -p FIFO -l 200,200,200 -c
ARG policy FIFO
ARG jlist 200,200,200
...
*Q:Now do the same but with jobs of different lengths: 100, 200, and
300.*

A:

Still the same because they are ranked by length from short to long.

* SJF:
```
E:\proj\cs\hw5600\hw2>python ./scheduler.py -p SJF -l 100,200,300 -c
ARG policy SJF
...
* FIFO:
```
E:\proj\cs\hw5600\hw2>python ./scheduler.py -p FIFO -l 100,200,300 -c
ARG policy FIFO
ARG jlist 100,200,300
...
*Q:Now do the same, but also with the RR scheduler and a time-slice
of 1.*

A: The running result is like this.

```
E:\proj\cs\hw5600\hw2>python ./scheduler.py -p RR -l 100,200,300 -c -q 1
ARG policy RR
ARG jlist 100,200,300
...
*Q:For what types of workloads does SJF deliver the same turnaround
times as FIFO?*

A: As we discussed in Q2, if the incoming jobs are ranked by their job length (or the workload) from shortest to longest, then they have the same turnaround time because short jobs always come first, equivalent to the first-job-first.

### Q5

*Q:For what types of workloads and quantum lengths does SJF deliver
the same response times as RR?*

A: When the quantum length is not shorter than the largest job workload. Then RR has the same effect as SJF because every job can be completed in one slice.
...
A: The response time would increase as job lengths increase.

```
E:\proj\cs\hw5600\hw2>python ./scheduler.py -p SJF -l 300,200,200 -c
ARG policy SJF
ARG jlist 300,200,200
...
Execution trace:
  [ time   0 ] Run job 1 for 200.00 secs ( DONE at 200.00 )
  [ time 200 ] Run job 2 for 200.00 secs ( DONE at 400.00 )
  [ time 400 ] Run job 0 for 300.00 secs ( DONE at 700.00 )

Final statistics:
  Job   1 -- Response: 0.00  Turnaround 200.00  Wait 0.00
  Job   2 -- Response: 200.00  Turnaround 400.00  Wait 200.00
  Job   0 -- Response: 400.00  Turnaround 700.00  Wait 400.00

  Average -- Response: 200.00  Turnaround 433.33  Wait 200.00


E:\proj\cs\hw5600\hw2>python ./scheduler.py -p SJF -l 400,200,200 -c
ARG policy SJF
ARG jlist 400,200,200
...
Execution trace:
  [ time   0 ] Run job 1 for 200.00 secs ( DONE at 200.00 )
  [ time 200 ] Run job 2 for 200.00 secs ( DONE at 400.00 )
  [ time 400 ] Run job 0 for 400.00 secs ( DONE at 800.00 )

Final statistics:
  Job   1 -- Response: 0.00  Turnaround 200.00  Wait 0.00
  Job   2 -- Response: 200.00  Turnaround 400.00  Wait 200.00
  Job   0 -- Response: 400.00  Turnaround 800.00  Wait 400.00

  Average -- Response: 200.00  Turnaround 466.67  Wait 200.00


E:\proj\cs\hw5600\hw2>python ./scheduler.py -p SJF -l 400,500,200 -c
ARG policy SJF
ARG jlist 400,500,200
...
Execution trace:
  [ time   0 ] Run job 2 for 200.00 secs ( DONE at 200.00 )
  [ time 200 ] Run job 0 for 400.00 secs ( DONE at 600.00 )
  [ time 600 ] Run job 1 for 500.00 secs ( DONE at 1100.00 )

Final statistics:
  Job   2 -- Response: 0.00  Turnaround 200.00  Wait 0.00
  Job   0 -- Response: 200.00  Turnaround 600.00  Wait 200.00
  Job   1 -- Response: 600.00  Turnaround 1100.00  Wait 600.00

  Average -- Response: 266.67  Turnaround 633.33  Wait 266.67

```

### Q7

*Q: What happens to response time with RR as quantum lengths increase? Can you write an equation that gives the worst-case response time, given N jobs?*

A: Given `n` jobs, the response time would also rise as quantum lengths increase. <br />
In the worst case, the sum of response time would be `(0 + 1 + 2 + ... + n - 1) * Ql`, where `Ql` stands for the quantum length. <br />
Therefore the average response time would be `(0 + n - 1) * n * Ql / 2 / n` = `(n - 1) * Ql / 2`.
